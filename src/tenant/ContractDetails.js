    ...
    return (
        <React.Fragment>
            ...
            {targetData.contract.details.map((detail , index) => (
                ...
                <div className={classes.contractDetails} key={index} >
                    <FormControl variant="outlined" className={classes.formControl} >
                        <InputLabel className={classes.contractDetailsItem} htmlFor="outlined-age-native-simple">製品</InputLabel>
                        <Select
                        ...
                        >
                            {productLicenses.map((p) => (
                                <MenuItem value={p.id} key={p.id} >{p.caption}</MenuItem>
                            ))}
                        </Select>
                    ...
                    {targetData.contract.details.length > 1 ?
                        <IndeterminateCheckBoxIcon className={classes.contractDetailsItem}
                                               onClick={() => delDetailHandler(index)}/> : null}
                    {targetData.contract.details.length === (index + 1) ?
                        <AddBoxIcon className={classes.contractDetailsItem} onClick={addDetailHandler}/> : null}
                </div>
            ))}
            ...
            <TextField
                ...
                name="contract.remarks" onChange={uiToJson}
                label="契約特記事項"
                helperText="契約特記事項"
                multiline
                fullWidth
