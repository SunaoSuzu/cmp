...
const command = require("./ToCloudFormation");
const ret = command.convert(requestQue.vpc);
...
then(function (result) {
    ...
    if(exist){
        console.log("wait");
        return cloudformation.waitFor("stackDeleteComplete" , {StackName: stackName}).promise()
    }
}).then(function (result) {
    console.log("create");
    return cloudformation.createStack({
        StackName: stackName,
        TemplateBody : JSON.stringify(ret),
    }).promise();
}).then(function (result) {
